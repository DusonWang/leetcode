...
public class MaxVacationDays {
    ...
    public int maxVacationDays(int[][] flights, int[][] days) {
        int cites = flights.length;
        int weeks = days[0].length;
        int[][] dp = new int[weeks][cites];
        ...
        for (int i = 1; i < weeks; i++) {
            for (int city = 0; city < cites; city++) {
                for (int k = 0; k < cites; k++) {
                    if (flights[k][city] > 0 && dp[i - 1][k] >= 0) {
                        dp[i][city] = Math.max(dp[i][city], dp[i - 1][k] + days[city][i]);
                    }
                }
            }
        }
